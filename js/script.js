...
function getTeamMembers() {
    if (J.get('team')) {
        return (executeQuery(`select name from teamleden where teamId= ${J.get('team')}`));
    }
}
...
function klassementNIEUW(jaar){
  executeQuery(`select distinct game from scores`)
  .forEach(function(result){
    executeQuery(`select * from scores where game = ${result.game} group by hole order by hole`)
    .forEach(function(result){
    J.log(result);
        var teamRow = "";
        var totaal = 0;
        var team = result.team;
        var teamNaam = result.teamnaam;
        var game = result.game;
        var startHole = result.startHole;
...
function finishGame() {
    results.innerHTML = '';
    replay.innerHTML = '';
    renderTable(J.getTeamScore(), 'teamScore');
...
function getTeamScore() {
    var query = `SELECT s.hole,s.game,s.kleur,s.score,sum(s.score-h.par) as verschil,s.team as team FROM scores as s left join holes as h on h.hole=s.hole group by s.id having s.game =  ${J.get('game')} and team = ${J.get('team')}`;
